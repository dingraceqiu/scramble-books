...
- **轮换工具**：服务器 `/usr/local/sbin/scramble-books-key-rotation`（root `0700`；事实源 = 仓库 `ops/systemd/scramble-books-key-rotation.sh`）。三个子命令：
  - `apply`：隐藏输入新密钥（不进对话/命令行/日志）→ 轮换前 unit 备份到 `/root/scramble-books-rotation/`（0700 目录、0600 文件，**逐步校验，任何失败都在修改 unit/envfile 前终止；重复 apply 时副本与当前 unit 一致则复用、不同则拒绝覆盖**）→ 写 envfile → 更新 unit → 收紧 DB → `daemon-reload` + 受控重启 → 全量验证（health / auth-me / 页面 / **真实 GLM 探针须 HTTP 200 且 ok=true**，只输出状态与布尔 / **journalctl 执行失败按验证失败处理，绝不误判零命中** / envfile 完整 600 root:root 断言 / DB+WAL+SHM 均 0600 / 密钥卫生计数 / journal 增量零命中）。**启动失败与最终验证任一失败都自动回滚**（恢复旧 unit + daemon-reload + restart + 复核旧服务恢复）；退出码：0 成功 / 2 前置失败零修改 / 3 启动失败已回滚 / 4 验证失败已回滚（禁止撤销旧密钥）/ 5 回滚失败需人工。
