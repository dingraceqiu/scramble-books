...
- **轮换工具**：服务器 `/usr/local/sbin/scramble-books-key-rotation`（root `0700`；事实源 = 仓库 `ops/systemd/scramble-books-key-rotation.sh`）。三个子命令：
  - `apply`：隐藏输入新密钥（不进对话/命令行/日志）→ 轮换前 unit 备份到 `/root/scramble-books-rotation/`（0700 目录、0600 文件，**逐步校验，任何失败都在修改 unit/envfile 前终止；重复 apply 时副本与当前 unit 一致则复用、不同则拒绝覆盖**）→ 写 envfile → 更新 unit → 收紧 DB → `daemon-reload` + 受控重启 → 全量验证（health / auth-me / 页面 / **真实 GLM 探针重试 3 次（间隔 5s）、以响应中的 `generator` 字段为成功哨兵（仅 GLM 调用成功路径存在；不以 ok=true 为必要条件——GLM 对探针假 id 常返回索引式 id 导致应用静默丢弃结果，2026-09-13 三次误回滚的根因），失败时输出脱敏错误类别（不打印响应正文）** / **journalctl 执行失败按验证失败处理，绝不误判零命中** / envfile 完整 600 root:root 断言 / DB+WAL+SHM 均 0600 / 密钥卫生计数 / journal 增量零命中）。**启动失败与最终验证任一失败都自动回滚**（恢复旧 unit + daemon-reload + restart + 复核旧服务恢复）；退出码：0 成功 / 2 前置失败零修改 / 3 启动失败已回滚 / 4 验证失败已回滚（禁止撤销旧密钥）/ 5 回滚失败需人工。
  - `verify`：只验证当前状态，不需要密钥。
  - `cleanup`：**旧密钥在 GLM 控制台撤销成功之后**执行。前置强校验（服务运行中、unit 无内联密钥且含准确的 `EnvironmentFile=/etc/default/scramble-books` 与 `UMask=0077`、envfile root:root 0600、回滚目标为固定路径的 0600 普通文件且位于 0700 root-only 目录）+ **显式输入大写 `YES` 确认**；只删除这一个已校验文件，然后 `rmdir` 空目录——全脚本无任何 `rm -rf`。
- **顺序纪律**：apply 验证全绿 → 控制台撤销旧密钥 → cleanup 输入 YES。验证未全绿（退出码 4）时脚本已自动回滚，**禁止**撤销旧密钥。
- **注入守卫**：生产模式禁止任何 `SB_*` 环境变量（含空值）；测试必须显式 `SB_TEST_MODE=1` + `SB_TEST_ROOT`（非空、非 `/`），所有 `SB_*` 路径经 realpath 解析后必须严格位于测试根内（符号链接/相对路径逃逸均被拒）。
- **CI 门禁**：`.github/workflows/verify.yml` 的 `ops-hardening` job 在每个 PR 上跑 shellcheck + 全套故障注入测试（/tmp 隔离环境，覆盖 restart 立即失败、服务延迟退出、GLM/journal 验证失败回滚、副本创建失败零修改、cleanup 拒绝路径与危险路径注入）。
