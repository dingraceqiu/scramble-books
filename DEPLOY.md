...
- **轮换工具**：服务器 `/usr/local/sbin/scramble-books-key-rotation`（root `0700`；事实源 = 仓库 `ops/systemd/scramble-books-key-rotation.sh`）。三个子命令：
  - `apply`：隐藏输入新密钥（不进对话/命令行/日志）→ 轮换前 unit 备份到 `/root/scramble-books-rotation/`（0700 目录、0600 文件，**逐步校验，任何失败都在修改 unit/envfile 前终止**）→ 写 envfile → 更新 unit → 收紧 DB → `daemon-reload` + 受控重启 → 全量验证（health / auth-me / 页面 / 真实 GLM 探针只输出状态与布尔 / 密钥卫生计数 / journal 增量零命中）。**启动失败与最终验证任一失败都自动回滚**（恢复旧 unit + daemon-reload + restart + 复核旧服务恢复）；退出码：0 成功 / 2 前置失败零修改 / 3 启动失败已回滚 / 4 验证失败已回滚（禁止撤销旧密钥）/ 5 回滚失败需人工。
  - `verify`：只验证当前状态，不需要密钥。
  - `cleanup`：**旧密钥在 GLM 控制台撤销成功之后**执行。前置强校验（服务运行中、unit 无内联密钥、envfile root:root 0600、回滚目标为固定路径的 0600 普通文件且位于 0700 root-only 目录）+ **显式输入大写 `YES` 确认**；只删除这一个已校验文件，然后 `rmdir` 空目录——全脚本无任何 `rm -rf`。
