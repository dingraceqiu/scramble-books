...
- **已知残留风险**：限流状态为单进程内存态，服务重启即重置（可接受的软状态）。~~5000 端口进程侧监听 `*`~~ **已收紧（2026-09-14 独立 hardening）**：Express 默认只绑 `127.0.0.1`（`LISTEN_HOST` 环境变量，unit 与 rotation unit 模板同步更新），公网仅经本机 Nginx 反代进入，不再依赖云安全组。
