...
- **已落地方案（2026-09-14，多层内存态守卫 `server/lib/abuseGuard.ts`）**：
  ...
  2. **多层限制**（全部环境变量可调、不含密钥）：每桶每分钟频率（默认 30）、每桶每日额度（默认 400）、全局每日 GLM 硬上限（默认 3000）、桶级/全局并发（默认 4/16）、探针独立小桶（默认 20/日、6/分）。被拒请求返回稳定 JSON 429 + `Retry-After`，**绝不触达 GLM 上游**。
...
- **测试**：`pnpm verify:abuse-guard`（57 项断言，已入 `pnpm verify` 链与 GitHub Verify CI）；覆盖三接口保护、合法放行、429/413/400 边界、每日/并发/伪造头/登录-匿名/探针隔离与诚实失败、拒绝请求 GLM mock 调用为 0、响应无内容泄露、原路由不受影响。
