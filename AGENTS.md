...
- **阅读状态（readRanges 事实层）**：`ReadingProgress.readRanges`（chapterId + 节点闭区间 + via: 'feed'|'reader' + at）是「我读过哪些原文」的唯一事实来源；`readUnitIds` 只是由它推导的呈现层缓存，禁止单独写入。Feed（单元已读 = 区间被完全覆盖，`readState.isUnitRead`）、Reader（IntersectionObserver 上报 `markNodesRead`，按章合并区间）、覆盖率（`coverageOf`）、Quiz 出题范围全部从 readRanges 推导；相关纯函数集中在 `src/lib/readState.ts`。旧数据（只有 readUnitIds）在 loadAll 的 `normalizeProgress` 自动迁移（存量区间一律 `mergeReadRanges` 后使用，不信任写入方有序性）。**重切分语义分两类**：presentation re-segmentation（`setBookType`，documents 不变）保留 readRanges；source re-parsing（重新解析原文）必须先做迁移（见 TECH-DEBT.md TD-05），禁止混用 `preserveReadRanges`。
